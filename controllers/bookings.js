...
const getSingle = async (req, res) => { // GET Request
  ...
  if (!ObjectId.isValid(req.params.id)) {
    return res.status(400).json('Must have a valid booking id to get a single booking');
  }
...
const updateBooking = async (req, res) => { // PUT Request
  //#swagger.tags=['Bookings']
  if (!ObjectId.isValid(req.params.id)) {
    return res.status(400).json('You must have a valid booking id to update a booking');
  }
  const bookingId = new ObjectId(req.params.id);
  const booking = {
    hotelId: req.body.hotelId,
...
const deleteBooking = async (req, res) => { // DELETE Request
  //#swagger.tags=['Bookings']
  if (!ObjectId.isValid(req.params.id)) {
    return res.status(400).json('You must have a valid booking id to delete a booking');
  }
  const bookingId = new ObjectId(req.params.id);
  const response = await mongodb
    .getDatabase()
